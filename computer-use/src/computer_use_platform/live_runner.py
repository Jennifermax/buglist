...
class LiveCaseRunner:
    def __init__(self, case: Dict[str, Any], artifacts_dir: str = "artifacts"):
        self.case = case
        ...
        with sync_playwright() as p:
            ...
            try:
                for index, step in enumerate(self.case.get("steps", []), start=1):
                    status = self._run_step(page, step, index)
                    if status == "fail":
    ...
    def _run_step(self, page: Page, step: Dict[str, Any], step_no: int) -> str:
        ...
        try:
            ...
            if action == "验证":
                return self._run_validation(page, step_id, description, value, semantic_evidence)
