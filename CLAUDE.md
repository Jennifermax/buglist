...
│       └── api.js              # API 配置
└── backend/                     # FastAPI 后端
    ├── main.py                 # 应用入口（含禅道自动初始化）
    ├── routers/                 # API 路由
    │   └── zentao.py          # 禅道 API 路由
    ├── services/                # 服务层
    │   ├── zentao_service.py   # 禅道 API 服务
    │   └── zentao_init.py      # 禅道初始化服务
    └── models/                  # 数据模型
        └── config.py           # 配置模型（含禅道配置）
```
...
#### 禅道集成
```
POST /api/zentao/test-connection   # 测试禅道连接
GET  /api/zentao/products          # 获取产品列表
GET  /api/zentao/products/{id}     # 获取产品详情
GET  /api/zentao/products/{id}/modules  # 获取产品模块
GET  /api/zentao/bugs              # 获取 Bug 列表
POST /api/zentao/bugs               # 创建 Bug
GET  /api/zentao/users             # 获取用户列表
```

##### 禅道配置
禅道配置支持两种方式：
1. **自动初始化**：后端启动时从 `login.txt` 自动读取配置并登录
2. **手动配置**：通过前端设置页面配置

##### login.txt 格式
```
禅道网址：
https://cfazendao.chandao.net/bug-browse-1.html
...
- **禅道地址**: 禅道服务器地址
- **账号**: 禅道账号
- **密码**: 禅道密码（用于自动获取 Token）
- **Token**: API Token（可选，自动获取）

#### 禅道自动初始化
后端服务启动时会自动：
1. 读取 `login.txt` 文件获取禅道配置
2. 通过账号密码登录禅道获取 Token
3. 保存配置到 `data/config.json`
4. 测试连接并创建测试 Bug
...
- 获取产品列表
- 获取 Bug 列表
- 创建 Bug（支持 title, product, pri, severity, type, steps 等字段）
...
```
后端启动时会自动从 `login.txt` 读取禅道配置，登录并创建测试 Bug。
